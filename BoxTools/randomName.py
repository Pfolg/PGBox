...
def window(text):
    ...
    ttk.Label(
        w, text="恭喜:", font=("微软雅黑", 20), background="#66bdff",
        foreground="#FFFFFF").place(relx=.1, rely=.1)
    ttk.Label(
        w, text=text + " 同学", font=("微软雅黑", 30), compound="center",
        foreground="#FFFFFF", background="#66bdff").place(relx=.3, rely=.2)
    tk.Button(w, text="确定", command=lambda: w.destroy(), width=8).place(relx=.4, rely=.7)
...
def randomName(frame):
    ...
    ttk.Button(frame, text="选择", command=lambda: find_files(file)).place(relx=.7, rely=.398)
    ttk.Button(frame, text="开始", command=lambda: window(showName(filePath.get()))).place(relx=.43, rely=.6)

    ttk.Button(
        frame, text="帮助", command=lambda: messagebox.showinfo(title="帮助", message=randomName_information),
        width=8).place(relx=.8, rely=.9)

    ttk.Label(
        frame, text="你不能怪作者，作者也是被**毒害了的人，肯定要回馈一下**啊(手动狗头)",
