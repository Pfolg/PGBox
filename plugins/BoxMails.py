...
def showHistory(inf):
    if inf:
        ...
        while True:
            ...
            # 遍历邮件
            print()
            showMail.insert(tk.END, f"<{abs(i)}>" + "-" * 10 + "\n")
            # 获取邮件的详细信息
            ...
            try:
                ...
                for j in from_header:
                    listJ.append(j[1])
                print('From:', *listJ)
                showMail.insert(tk.END, f"From:   {listJ}\n")
                to_header = email.utils.getaddresses([msg['To']])  # [0][1]
                liatK = []
                for k in to_header:
                    liatK.append(k[1])
                print('To:', *liatK)
                showMail.insert(tk.END, f"To:   {liatK}\n")
                print("~~~")
                showMail.insert(tk.END, "~~~\n")
...
def mainMail(frame):
    ...
    writeMail = tk.Text(frame, height=16, width=70)
    ...
    # 只读文本框，查看邮件
    showMail = tk.Text(frame, height=20, width=70)
